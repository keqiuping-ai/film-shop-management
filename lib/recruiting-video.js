  ...
  async function handleAdmin(req, res, url, actor) {
    ...
    try {
      ...
      if (action === 'analyze') {
        ...
        const analysisInvite = activeInvite(db, interviewId), evidenceKey = evidenceFingerprint(interview);
        if (mode === 'final') auto.assertReadyForAnalysis(interview, analysisInvite);
        ...
        if (mode === 'final') {
          ...
          if ((latestInvite?.id || '') !== (analysisInvite?.id || '') || ['completed','cancelled'].includes(latest.status)) fail(409, '面试会话已变化，本次分析未保存', 'INTERVIEW_ANALYSIS_STALE');
          auto.assertReadyForAnalysis(latest, latestInvite);
