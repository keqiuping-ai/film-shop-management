...
function aiState(interview, includeAnalysis = false) {
  ...
  const result = { transcript:Array.isArray(state.transcript) ? state.transcript.slice(-300) : [] };
  if (includeAnalysis) result.analysis = state.analysis || null;
  return result;
}
function addTranscript(interview, speaker, body) {
  ...
  const duplicate = interview.aiInterview.transcript.some(row => row.id === String(body?.id || ''));
  if (!duplicate) interview.aiInterview.transcript.push({ id:String(body?.id || uid()), speaker, text:value,
  ...
  async function handlePublic(req, res, url) {
    ...
    try {
      ...
      if (sessionRoute || transcriptRoute) {
        ...
        if (transcriptRoute) {
          const { interview } = interviewContext(db, invite);
          addTranscript(interview, 'candidate', body); writeDb(db); notify('recruiting-video-transcript', invite.interviewId);
      ...
      if (operation === 'exchange' && req.method === 'POST') {
        ...
        send(res, 200, { ...publicInfo(db, invite), aiState:aiState(interviewContext(db, invite).interview), sessionSecret:secret, ...access });
        return true;
      }
      fail(405, '此操作不支持');
  ...
  async function handleAdmin(req, res, url, actor) {
    ...
    try {
      ...
      if (action === 'transcript') {
        addTranscript(interview, 'interviewer', await readBody(req));
        save(db, 'recruiting-video-transcript', interviewId); send(res, 201, { ok:true, aiState:aiState(interview, true) }); return true;
      }
      if (action === 'analyze') {
        if (typeof analyzeInterview !== 'function') fail(503, 'AI 面试分析尚未配置', 'INTERVIEW_AI_NOT_CONFIGURED');
        const body = await readBody(req); const mode = body?.mode === 'final' ? 'final' : 'next';
        const analysis = await analyzeInterview({ candidate, interview, transcript:aiState(interview).transcript, mode });
        if (!interview.aiInterview || typeof interview.aiInterview !== 'object') interview.aiInterview = { transcript:[] };
        interview.aiInterview.analysis = { ...analysis, generatedByUserId:actor.id, generatedBy:actor.name || actor.email || '' };
        save(db, `recruiting-video-ai-${mode}`, interviewId); send(res, 200, { aiState:aiState(interview, true) }); return true;
      }
      if (action === 'invite') {
      ...
      if (action === 'token') {
        send(res, 200, { interviewId, candidateName:candidate.name, startsAt:interview.startsAt,
          ...
          interviewKits:INTERVIEW_KITS, aiState:aiState(interview, true),
