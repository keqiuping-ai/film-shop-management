...
const { createRecruitingVoiceService, consented } = require('./recruiting-voice');
  ...
  async function handlePublic(req, res, url) {
    ...
    try {
      ...
      if (sessionRoute || transcriptRoute) {
        ...
        if (transcriptRoute) {
          if (!consented(invite)) fail(409, '须由候选人单独同意 AI 语音与转写后才能保存', 'INTERVIEW_VOICE_CONSENT_REQUIRED');
          const { interview, candidate } = interviewContext(db, invite);
  ...
  async function handleAdmin(req, res, url, actor) {
    ...
    try {
      ...
      if (action === 'transcript') {
        if (!consented(activeInvite(db, interviewId))) fail(409, '须由候选人单独同意 AI 语音与转写后才能保存', 'INTERVIEW_VOICE_CONSENT_REQUIRED');
