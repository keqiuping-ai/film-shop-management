...
const { emailAddress, emailSender, emailFailure } = require('./recruiting-email');

const TIME_ZONE = 'America/Los_Angeles';
const ADDRESS = '3212 Santa Monica Blvd, Santa Monica, CA 90404';
const SMS_SHARED_PHONE_TEST_HOURS = 24;
...
const TEXT_FIELDS = { name:160, phone:40, email:254, source:100, position:160, location:240, experience:8000, dealershipResources:8000, availability:2000, employmentType:100, compensation:2000, notes:8000, resumeText:60000, resumeUrl:2000, scoreNotes:8000, smsConsentNote:2000, emailOptOutNote:2000, applicationDateNote:1000 };
const CANDIDATE_FIELDS = new Set([...Object.keys(TEXT_FIELDS), 'status', 'scores', 'smsConsent', 'emailOptedOut', 'appliedAt']);
...
function customerPhoneOptedOut(db, phone) {
  const key = phoneKey(phone);
  return Boolean(key && ['customerConversations', 'prospects', 'portalCustomers', 'jobs'].some(collection =>
    (db[collection] || []).some(row => phoneKey(row.phone || row.contactPhone || row.customerPhone) === key
      && (row.smsOptedOut === true || Boolean(row.marketingOptOutAt)))));
}
// A reversible owner-approved test window, bound to one record and exact phone.
// This does not grant SMS consent or opt the recipient back in.
function sharedPhoneTestActive(db, candidate, eventAt = Date.now()) {
  const test = candidate?.smsSharedPhoneTest;
  const now = Date.now(), eventTime = typeof eventAt === 'number' ? eventAt : Date.parse(eventAt);
  const start = Date.parse(test?.approvedAt), end = Date.parse(test?.expiresAt);
  return Boolean(test?.enabled === true && test.candidateId === candidate.id
    && typeof test.phoneKey === 'string' && test.phoneKey && test.phoneKey === phoneKey(candidate.phone)
    && typeof test.approvedByUserId === 'string' && test.approvedByUserId && test.approvedByRole === 'owner'
    && typeof test.reason === 'string' && test.reason.trim()
    && Number.isFinite(start) && Number.isFinite(end) && end > start
    && end - start <= SMS_SHARED_PHONE_TEST_HOURS * 3600000
    && now >= start && now < end && eventTime >= start && eventTime < end
    && candidatesByPhone(db, candidate.phone).length === 1);
}
function candidateSnapshot(db, candidate) {
  return { ...candidate, emailMessages:Array.isArray(candidate.emailMessages) ? candidate.emailMessages : [],
    emailOptedOut:Boolean(candidate.emailOptedOut), smsCustomerPhoneConflict:Boolean(customerPhoneConflict(db, candidate.phone)),
    smsCustomerOptedOut:customerPhoneOptedOut(db, candidate.phone),
    smsSharedPhoneTestActive:sharedPhoneTestActive(db, candidate) };
}
function privateAudit(db, actor, action, recordId, at = new Date().toISOString(), details = null) {
  ensure(db).recruitingAudit.unshift({ id:uid(), at, userId:actor?.id || 'system', userName:actor?.name || 'System', action, recordId, ...(details ? { details } : {}) });
...
function normalizeCandidate(db, body, actor, existing = null) {
  ...
  const item = existing ? { ...existing } : { id:uid(), status:'new', scores:Object.fromEntries(SCORE_KEYS.map(key => [key, null])), interviewScorecards:[], smsConsent:false, smsConsentNote:'', smsOptedOut:false, messages:[], emailMessages:[], emailOptedOut:false };
  ...
  if (Object.hasOwn(body, 'emailOptedOut')) {
    if (typeof body.emailOptedOut !== 'boolean') fail(400, '邮件拒收设置须为是或否');
    item.emailOptedOut = body.emailOptedOut;
    if (body.emailOptedOut && !existing?.emailOptedOut) item.emailOptedOutAt = now;
  }
  ...
  if (existing && phoneKey(existing.phone) !== phoneKey(item.phone)) {
    ...
    if (item.smsSharedPhoneTest) item.smsSharedPhoneTest = { ...item.smsSharedPhoneTest, enabled:false, revokedAt:now };
  }
...
function ingestInbound(db, params) {
  ...
  const sharedPhone = customerPhoneConflict(db, params.From || params.from);
  const testRoute = matches.length === 1 && sharedPhone && sharedPhoneTestActive(db, matches[0], at);
  if (matches.length !== 1 || (sharedPhone && !testRoute)) {
    ...
    if (testRoute) message.sharedPhoneTest = { approvedAt:candidate.smsSharedPhoneTest.approvedAt, approvedByUserId:candidate.smsSharedPhoneTest.approvedByUserId };
    candidate.messages = [...(candidate.messages || []), message]; candidate.updatedAt = at;
    privateAudit(db, null, testRoute ? 'receive-recruiting-test-sms' : 'receive-recruiting-sms', candidate.id);
  }
...
function createRecruitingService(deps) {
  ...
  function emailInfo() {
    let info;
    try { info = deps.emailInfo?.(); } catch { info = null; }
    const from = emailSender(info?.from), replyTo = emailAddress(info?.replyTo);
    return { configured:Boolean(info?.configured === true && from && replyTo && typeof deps.sendEmail === 'function'), from, replyTo, inboundConfigured:false,
      configuration:{ providerConfigured:info?.configuration?.providerConfigured === true, senderConfigured:Boolean(from), replyToConfigured:Boolean(replyTo) } };
  }
  function changed(db, actor, action, recordId, details = null) {
    privateAudit(db, actor, action, recordId, undefined, details); writeDb(db);
  ...
  function snapshot(db) {
    ...
    return { candidates:db.recruitingCandidates.map(candidate => candidateSnapshot(db, candidate)), interviews:db.recruitingInterviews.map(interview => ({ ...interview, mode:interview.mode || 'in_person' })), interviewKits:INTERVIEW_KITS,
      settings:{ address:ADDRESS, timeZone:TIME_ZONE, remindersEnabled:remindersEnabled(), preview:process.env.RECRUITING_PREVIEW === 'true', smsSharedPhoneTestHours:SMS_SHARED_PHONE_TEST_HOURS },
      sms:{ configured:smsConfigured() }, email:emailInfo(), translation:{ configured:translation.configured() }, quarantine:db.recruitingQuarantine,
  ...
  async function sendCandidateMessage(candidateId, body, actor, callbackBase = '', reminder = null) {
    ...
    const sharedPhone = customerPhoneConflict(db, candidate.phone);
    if (sharedPhone && customerPhoneOptedOut(db, candidate.phone)) fail(409, '同号客户资料存在退订记录，测试开关不能解除退订', 'SMS_OPTED_OUT');
    const testSend = sharedPhone && !reminder && sharedPhoneTestActive(db, candidate);
    if (sharedPhone && !testSend) fail(409, '该号码也在客户资料中。请先核实；如为本人测试号码，可由老板在短信窗口开启限时测试，其他号码不放行', 'PHONE_CUSTOMER_CONFLICT');
    ...
    if (testSend) {
      message.sharedPhoneTest = { approvedAt:candidate.smsSharedPhoneTest.approvedAt, approvedByUserId:candidate.smsSharedPhoneTest.approvedByUserId };
      privateAudit(db, actor, 'send-recruiting-test-sms', candidate.id);
    }
  ...
  async function sendCandidateEmail(candidateId, body, actor) {
    validateFields(body, new Set(['subject', 'text', 'expectedEmail', 'clientMessageId']));
    const subject = text(body.subject, 200, '邮件主题'), messageText = text(body.text, 10000, '邮件正文');
    if (!subject || !messageText || /[\r\n\x00-\x1f\x7f]/.test(body.subject)) fail(400, '邮件主题和正文不能为空，主题不能包含换行或控制字符', 'EMAIL_VALIDATION');
    if (HAN_TEXT.test(subject + messageText)) fail(400, '招聘邮件的主题和正文须为英文，请先核对英文预览后发送', 'EMAIL_ENGLISH_REQUIRED');
    if (typeof body.clientMessageId !== 'string' || !/^[A-Za-z0-9_-]{8,100}$/.test(body.clientMessageId)) fail(400, '缺少有效的邮件发送编号，请刷新后重试', 'EMAIL_VALIDATION');
    const expectedEmail = emailAddress(body.expectedEmail);
    if (!expectedEmail) fail(400, '请先核对单个有效的收件邮箱', 'EMAIL_RECIPIENT_INVALID');
    let db = ensure(readDb());
    const candidate = candidateById(db, candidateId), to = emailAddress(candidate.email);
    if (!to || to !== expectedEmail) fail(409, '应聘者邮箱已变更或无效，请重新核对收件邮箱与英文预览', 'EMAIL_RECIPIENT_CHANGED');
    const records = Array.isArray(candidate.emailMessages) ? candidate.emailMessages : [];
    const existing = records.find(row => row.direction === 'outbound' && row.clientMessageId === body.clientMessageId);
    if (existing) {
      if (existing.to !== to || existing.subject !== subject || existing.text !== messageText) fail(409, '该发送编号已用于其他收件人或邮件内容，请刷新后核查', 'EMAIL_MESSAGE_ID_CONFLICT');
      return { message:existing, duplicate:true };
    }
    if (candidate.emailOptedOut || candidate.doNotContact) fail(409, '应聘者已明确拒收邮件或要求不再联系，请勿发送', 'EMAIL_OPTED_OUT');
    if (db.recruitingCandidates.filter(row => emailAddress(row.email) === to).length !== 1) fail(409, '该邮箱属于多个应聘者档案，请先核实', 'EMAIL_AMBIGUOUS');
    const channel = emailInfo();
    if (!channel.configured) throw emailFailure('EMAIL_NOT_CONFIGURED');
    const recent = records.find(row => row.direction === 'outbound' && row.to === to && row.subject === subject && row.text === messageText
      && (['pending', 'send_unknown'].includes(row.status) || (row.status === 'accepted' && Date.now() - Date.parse(row.timestamp) < 2 * 60000)));
    if (recent) return { message:recent, duplicate:true };
    const now = new Date().toISOString();
    const message = { id:uid(), channel:'email', direction:'outbound', clientMessageId:body.clientMessageId,
      subject, text:messageText, to, from:channel.from, replyTo:channel.replyTo, timestamp:now,
      status:'pending', provider:'resend', providerId:'', createdByUserId:actor.id, createdBy:actor.name || actor.email || '' };
    const idempotencyKey = `recruiting-email-${crypto.createHash('sha256').update(JSON.stringify([candidateId, body.clientMessageId])).digest('hex')}`;
    candidate.emailMessages = [...records, message]; candidate.updatedAt = now;
    changed(db, actor, 'send-recruiting-email-pending', candidateId);
    try {
      const result = await deps.sendEmail({ to, subject, text:messageText, idempotencyKey });
      if (typeof result?.id !== 'string' || !/^[A-Za-z0-9_-]{1,200}$/.test(result.id)) throw emailFailure('EMAIL_SEND_UNKNOWN');
      db = ensure(readDb());
      const latest = candidateById(db, candidateId), saved = latest.emailMessages?.find(row => row.id === message.id);
      if (!saved) throw emailFailure('EMAIL_SEND_UNKNOWN');
      saved.providerId = result.id; saved.status = 'accepted'; saved.acceptedAt = new Date().toISOString();
      latest.updatedAt = saved.acceptedAt;
      changed(db, actor, 'send-recruiting-email-accepted', candidateId);
      return { message:saved, duplicate:false };
    } catch (error) {
      const safe = emailFailure(error?.code);
      db = ensure(readDb());
      const latest = candidateById(db, candidateId), saved = latest.emailMessages?.find(row => row.id === message.id);
      if (saved) {
        if (saved.providerId && saved.status === 'accepted') return { message:saved, duplicate:false };
        saved.status = safe.providerRejected ? 'failed' : 'send_unknown';
        saved.errorCode = safe.code; saved.error = safe.message;
        latest.updatedAt = new Date().toISOString();
        changed(db, actor, 'send-recruiting-email-error', candidateId);
        safe.messageRecord = saved;
      }
      throw safe;
    }
  }
  async function handle(req, res, url, actor) {
    ...
    try {
      ...
      const route = url.pathname.match(/^\/api\/recruiting\/(candidates|interviews)(?:\/([a-zA-Z0-9_-]+))?(?:\/(messages|email-messages|messages-read|resume|scorecard|test-sms))?$/);
      if (!route) fail(404, '没有找到招聘功能');
      const [, collection, recordId, action] = route;
      if (action === 'test-sms' && collection === 'candidates' && recordId && req.method === 'POST') {
        if (actor?.role !== 'owner') fail(403, '只有老板可确认本人测试号码', 'SMS_TEST_OWNER_REQUIRED');
        const body = await readBody(req);
        validateFields(body, new Set(['enabled', 'expectedPhone', 'reason']));
        if (typeof body.enabled !== 'boolean') fail(400, '测试开关格式不正确');
        const reason = text(body.reason, 1000, '测试说明');
        const expectedPhone = text(body.expectedPhone, 40, '测试号码');
        const db = ensure(readDb()), candidate = candidateById(db, recordId), key = phoneKey(candidate.phone);
        if (!key || phoneKey(expectedPhone) !== key) fail(409, '测试号码已变化，请重新核对档案', 'SMS_RECIPIENT_CHANGED');
        const now = new Date().toISOString();
        if (body.enabled) {
          if (!reason) fail(400, '请说明该号码属于本人、仅用于招聘短信测试', 'SMS_TEST_REASON_REQUIRED');
          if (candidatesByPhone(db, candidate.phone).length !== 1) fail(409, '同号有多个应聘者档案，请先核实', 'PHONE_AMBIGUOUS');
          if (!candidate.smsConsent || !candidate.smsConsentNote?.trim()) fail(409, '请先记录实际取得的短信授权及依据', 'SMS_CONSENT_REQUIRED');
          if (candidate.smsOptedOut) fail(409, '该号码已退订，测试开关不能解除退订', 'SMS_OPTED_OUT');
          if (customerPhoneOptedOut(db, candidate.phone)) fail(409, '同号客户资料存在退订记录，测试开关不能解除退订', 'SMS_OPTED_OUT');
          candidate.smsSharedPhoneTest = { enabled:true, candidateId:candidate.id, phoneKey:key, reason,
            approvedByUserId:actor.id, approvedByRole:'owner', approvedAt:now,
            expiresAt:new Date(Date.parse(now) + SMS_SHARED_PHONE_TEST_HOURS * 3600000).toISOString() };
        } else {
          candidate.smsSharedPhoneTest = { ...(candidate.smsSharedPhoneTest || {}), enabled:false,
            revokedAt:now, revokedByUserId:actor.id, revokeReason:reason };
        }
        candidate.updatedAt = now;
        changed(db, actor, body.enabled ? 'enable-recruiting-test-sms' : 'revoke-recruiting-test-sms', candidate.id, { ...candidate.smsSharedPhoneTest });
        send(res, 200, { candidate:candidateSnapshot(db, candidate) }); return true;
      }
      if (action === 'messages-read' && collection === 'candidates' && recordId && req.method === 'POST') {
        ...
        send(res, 200, { candidate:candidateSnapshot(db, candidate), marked }); return true;
      }
      ...
      if (action === 'email-messages' && collection === 'candidates' && recordId && req.method === 'POST') {
        const result = await sendCandidateEmail(recordId, await readBody(req), actor);
        send(res, result.duplicate ? 200 : 201, result); return true;
      }
      if (action === 'scorecard' && collection === 'candidates' && recordId && req.method === 'POST') {
        ...
        send(res, 200, { candidate:candidateSnapshot(db, candidate), scorecard }); return true;
      }
      ...
      send(res, existing ? 200 : 201, { [collection === 'candidates' ? 'candidate' : 'interview']:collection === 'candidates' ? candidateSnapshot(db, item) : item });
    } catch (error) {
  ...
  return { handle, snapshot, sendCandidateMessage, sendCandidateEmail, processReminders, startReminderWorker };
}

module.exports = { createRecruitingService, normalizeCandidate, normalizeInterview, normalizeInterviewScorecard, ingestInbound, ingestStatus, hasCandidatePhone, phoneKey, customerPhoneConflict, sharedPhoneTestActive, ensure, TIME_ZONE, ADDRESS };
