...
function candidateSnapshot(db, candidate) {
  return { ...candidate, smsCustomerPhoneConflict:Boolean(customerPhoneConflict(db, candidate.phone)),
...
function createRecruitingService(deps) {
  ...
  async function handle(req, res, url, actor) {
    ...
    try {
      ...
      const route = url.pathname.match(/^\/api\/recruiting\/(candidates|interviews)(?:\/([a-zA-Z0-9_-]+))?(?:\/(messages|messages-read|resume|scorecard|test-sms))?$/);
