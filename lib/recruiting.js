...
const TEXT_FIELDS = { name:160, phone:40, email:254, source:100, position:160, location:240, experience:8000, dealershipResources:8000, availability:2000, employmentType:100, compensation:2000, notes:8000, resumeText:60000, resumeUrl:2000, scoreNotes:8000, smsConsentNote:2000 };
const CANDIDATE_FIELDS = new Set([...Object.keys(TEXT_FIELDS), 'status', 'scores', 'smsConsent']);
const INTERVIEW_FIELDS = new Set(['candidateId', 'startsAt', 'durationMinutes', 'timeZone', 'address', 'interviewerId', 'interviewerName', 'status', 'notes', 'automaticReminders']);
...
function normalizeCandidate(db, body, actor, existing = null) {
  ...
  const item = existing ? { ...existing } : { id:uid(), status:'new', scores:Object.fromEntries(SCORE_KEYS.map(key => [key, null])), smsConsent:false, smsConsentNote:'', smsOptedOut:false, messages:[] };
...
function normalizeInterview(db, body, actor, existing = null) {
  ...
  const item = { id:uid(), durationMinutes:30, timeZone:TIME_ZONE, address:ADDRESS, status:'scheduled', interviewerId:'', interviewerName:'', notes:'', automaticReminders:false, ...(existing || {}) };
  ...
  if (!item.address) fail(400, '请填写面试地址');
...
function createRecruitingService(deps) {
  ...
  function snapshot(db) {
    ...
    return { candidates:db.recruitingCandidates, interviews:db.recruitingInterviews,
      ...
      sms:{ configured:smsConfigured() }, quarantine:db.recruitingQuarantine,
  ...
  async function sendCandidateMessage(candidateId, body, actor, callbackBase = '', reminder = null) {
    validateFields(body, new Set(['text', 'clientMessageId']));
  ...
  async function handle(req, res, url, actor) {
    ...
    try {
      if (!canAccess(actor, 'recruitingView') || (req.method !== 'GET' && !canAccess(actor, 'recruitingEdit'))) fail(403, '没有招聘管理权限', 'RECRUITING_FORBIDDEN');
      if (req.method === 'GET' && url.pathname === '/api/recruiting') { send(res, 200, snapshot(readDb())); return true; }
      const route = url.pathname.match(/^\/api\/recruiting\/(candidates|interviews)(?:\/([a-zA-Z0-9_-]+))?(?:\/(messages|resume))?$/);
    ...
    try {
      ...
      for (const interview of interviews) {
        ...
        for (const hours of [24, 2]) {
          ...
          const body = { clientMessageId:key, text:`Hi ${candidate.name}, this is QUAD FILM. Reminder: your interview is scheduled for ${when} at ${interview.address}. Please reply to confirm or let us know if you need to reschedule. Reply STOP to opt out.` };
          const latestInterview = ensure(readDb()).recruitingInterviews.find(row => row.id === interview.id);
          if (!latestInterview?.automaticReminders || latestInterview.status !== 'confirmed' || latestInterview.startsAt !== interview.startsAt) { skipped++; continue; }
          try { const result = await sendCandidateMessage(candidate.id, body, { id:'recruiting-reminders', name:'招聘面试提醒' }, String(process.env.TWILIO_WEBHOOK_BASE_URL || '').replace(/\/$/, ''), { interviewId:interview.id, expectedStartsAt:interview.startsAt, hours }); if (!result.duplicate) sent++; }
          catch { skipped++; }
        }
      }
      return { sent, skipped };
...
module.exports = { createRecruitingService, normalizeCandidate, normalizeInterview, ingestInbound, ingestStatus, hasCandidatePhone, phoneKey, customerPhoneConflict, ensure, TIME_ZONE, ADDRESS };
