...
function createRecruitingService(deps) {
  ...
  function snapshot(db) {
    ...
    return { candidates:db.recruitingCandidates, interviews:db.recruitingInterviews, interviewKits:INTERVIEW_KITS,
  ...
  async function handle(req, res, url, actor) {
    ...
    try {
      ...
      const route = url.pathname.match(/^\/api\/recruiting\/(candidates|interviews)(?:\/([a-zA-Z0-9_-]+))?(?:\/(messages|resume|scorecard))?$/);
