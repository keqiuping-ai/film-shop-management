...
const handle=async(req,res)=>{
 ...
 if(p==='/api/health')return json(res,200,{ok:true,name:'哥俩好',voiceConfigured:calls.configured(),retentionHours:24,version:'2026.09.18.calls-24h'});
 ...
 const file=p==='/'?'index.html':p.slice(1);if(!['index.html','app.js','style.css','icon.svg','manifest.webmanifest','calls.js'].includes(file))fail(404,'页面不存在');
