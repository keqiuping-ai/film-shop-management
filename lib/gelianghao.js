...
CREATE TABLE IF NOT EXISTS password_resets(user_id TEXT PRIMARY KEY REFERENCES users(id),token_hash TEXT UNIQUE NOT NULL,expires INTEGER NOT NULL);
...
purgeExpired();
const cleanup=setInterval(()=>{try{const now=Date.now();for(const [k,v] of limits)if(v.end<now)limits.delete(k);run('DELETE FROM sessions WHERE expires < ?',now);run('DELETE FROM password_resets WHERE expires <= ?',now);purgeExpired();}catch{console.error('Private chat retention cleanup needs retry');}},60000).unref();
...
function issueReset(user){
 ...
 run('INSERT INTO password_resets(user_id,token_hash,expires) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET token_hash=excluded.token_hash,expires=excluded.expires',user.id,hash(token),expires);
 return {token,expires,username:user.username};
}
function resetRecord(token){
 if(typeof token!=='string'||!/^[a-f0-9]{64}$/.test(token))fail(400,'重置链接无效或已过期，请重新获取');
 const row=q('SELECT u.*,r.expires FROM password_resets r JOIN users u ON u.id=r.user_id WHERE r.token_hash=? AND r.expires>?',hash(token),Date.now());
 if(!row)fail(400,'重置链接无效或已过期，请重新获取');return row;
}
...
const handle=async(req,res)=>{
 ...
 if(p==='/api/health')return json(res,200,{ok:true,name:'哥俩好',voiceConfigured:calls.configured(),retentionHours:24,version:'2026.09.22.password-recovery'});
 ...
 // Recovery credentials are checked before disclosing any private account details.
 if(p==='/api/password-reset/owner'&&method==='POST'){
  limit(`recovery-owner:${req.socket.remoteAddress}`,5,600000);const b=await body(req);
  const email=String(b.ownerEmail||'').trim().toLowerCase();limit(`recovery-owner-account:${hash(email)}`,5,600000);
  if(!email||email.length>254||typeof b.ownerPassword!=='string'||!b.ownerPassword||b.ownerPassword.length>128||!options.authorizeSetup(req,{ownerEmail:email,ownerPassword:b.ownerPassword}))fail(403,'老板账号验证失败，请检查贴膜店账号及密码');
  const target=q("SELECT * FROM users WHERE username=? AND role='admin'",String(b.username||'').trim().toLowerCase());
  if(!target)fail(400,'未找到可恢复的哥俩好管理员账号');
  return json(res,201,issueReset(target));
 }
 if((p==='/api/password-reset/check'||p==='/api/password-reset/complete')&&method==='POST'){
  limit(`recovery-token:${req.socket.remoteAddress}`,40,600000);const b=await body(req),target=resetRecord(b.token);
  if(p.endsWith('/check'))return json(res,200,{username:target.username,name:target.name,expires:target.expires});
  const v=validateAccount({...target,password:b.password});
  if(b.password!==b.confirmPassword)fail(400,'两次输入的新密码不一致');
  const packed=packPassword(v.password);
  tx(()=>{resetRecord(b.token);run('UPDATE users SET password=? WHERE id=?',packed,target.id);run('DELETE FROM sessions WHERE user_id=?',target.id);run('DELETE FROM password_resets WHERE user_id=?',target.id);});
  closeUserStreams(target.id);return json(res,200,{ok:true});
 }
 if(p.startsWith('/api/')){
  ...
  if(p==='/api/password'&&method==='POST'){const b=await body(req),fresh=current(req);if(!fresh)fail(401,'请重新登录');limit(`password:${user.id}`,5,600000);if(String(b.oldPassword||'').length>128||!checkPassword(String(b.oldPassword||''),fresh.password))fail(400,'原密码不正确');const v=validateAccount({...user,password:b.password});tx(()=>{run('UPDATE users SET password=? WHERE id=?',packPassword(v.password),user.id);run('DELETE FROM sessions WHERE user_id=?',user.id);run('DELETE FROM password_resets WHERE user_id=?',user.id);});closeUserStreams(user.id);session(req,res,user);return json(res,200,{ok:true});}
  if(p==='/api/password-reset/issue'&&method==='POST'){
   if(user.role!=='admin')fail(403,'只有管理员能帮助朋友重置密码');
   limit(`recovery-issue:${user.id}`,5,600000);const b=await body(req),fresh=current(req);
   if(!fresh||fresh.role!=='admin')fail(401,'请重新登录');
   const password=String(b.adminPassword||'');
   if(password.length>128||!checkPassword(password,fresh.password))fail(403,'管理员密码不正确');
   if(b.identityConfirmed!==true)fail(400,'请先通过熟悉的联系方式核实朋友身份');
   const target=q("SELECT * FROM users WHERE id=? AND role='member'",String(b.userId||''));
   if(!target)fail(400,'请选择朋友账号；管理员请使用老板账号恢复');
   return json(res,201,issueReset(target));
  }
  if(p==='/api/invites'){
