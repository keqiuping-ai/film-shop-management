...
const { createRecruitingSpeechCache, SPEECH_PROFILE } = require('./recruiting-speech-cache');
const { AUTO_NOTICE_VERSION } = require('./recruiting-auto-transcript');
const VOICE_NOTICE_VERSION = AUTO_NOTICE_VERSION;
...
function createRecruitingVoiceProvider({ getConfig, fetchImpl = globalThis.fetch, timeoutMs = 30000 } = {}) {
  ...
  async function call(endpoint, body, isSpeech) {
    ...
    try {
      ...
      if (typeof value?.text !== 'string' || value.text.length > 12000) throw safeProviderError({ code:'INTERVIEW_VOICE_PROVIDER_INVALID' });
  ...
  return {
    ...
    speech:async ({ text }) => call('speech', JSON.stringify({ ...SPEECH_PROFILE, input:english(text) }), true),
...
function createRecruitingVoiceService(deps) {
  ...
  const speechCache = createRecruitingSpeechCache({ cacheDir:deps.speechCacheDir, generate:body => voiceProvider.speech(body) });
  ...
  function controller(ctx, actor, body, needsConsent = true) {
    ...
    if (needsConsent && !consented(ctx.invite)) reject('CONSENT_REQUIRED', '须由候选人选择允许 AI 语音及回答录音后才能开始');
    return identity;
  }
  ...
  function bankQuestion(kitId, questionId) {
    if (typeof kitId !== 'string' || typeof questionId !== 'string' || kitId.length > 100 || questionId.length > 100) reject('VALIDATION', '题库或题目编号无效', 400);
    const question = INTERVIEW_KITS.find(kit => kit.id === kitId)?.questions.find(question => question.id === questionId);
    if (!question) reject('VALIDATION', '题库或题目编号无效', 400);
    return english(question.en);
  }
  function rateLimitPrepare(actor) {
    const now = Date.now(), key = actor.id;
    const recent = (prepareAttempts.get(key) || []).filter(time => time > now - 60000);
    if (recent.length >= 90) reject('RATE_LIMIT', '语音准备请求过于频繁，请稍后再试', 429);
    prepareAttempts.delete(key); prepareAttempts.set(key, [...recent, now]);
    while (prepareAttempts.size > 5000) prepareAttempts.delete(prepareAttempts.keys().next().value);
  }
  ...
  async function handlePublic(req, res, url) {
    ...
    try {
      ...
      if (operation === 'consent') {
        ...
        deps.onCandidateConsent?.(ctx, body.consent, body.noticeVersion);
        // Any fresh consent event invalidates older in-flight audio work.
        if (body.consent !== consented(ctx.invite)) invalidate(ctx.state);
        ctx.invite.voiceConsent = { consent:body.consent, noticeVersion:VOICE_NOTICE_VERSION, method:'room_button', updatedAt:new Date().toISOString() };
  ...
  async function handleAdmin(req, res, url, actor) {
    const match = url.pathname.match(/^\/api\/recruiting\/interviews\/([A-Za-z0-9_-]+)\/video-(voice|speech|speech-prepare|followup|answer)$/);
    ...
    try {
      ...
      if (action === 'speech-prepare') {
        fields(body, ['kitId','questionId']);
        const text = bankQuestion(body.kitId, body.questionId), inviteId = ctx.invite.id;
        if (!configured()) reject('NOT_CONFIGURED', 'AI 语音服务尚未配置', 503);
        rateLimitPrepare(actor);
        // Preparing generic question audio does not join, speak, record, grant
        // consent, take control, or create a candidate answer/transcript.
        const audio = await speechCache.prepare(text);
        ctx = context(readDb(), interviewId); freshActor(ctx.db, actor);
        if (ctx.invite.id !== inviteId) reject('RESULT_STALE', '面试邀请已改变，请重新准备语音');
        if (!configured()) reject('NOT_CONFIGURED', 'AI 语音服务配置已改变，请重新准备语音', 503);
        send(res, 200, audio); return true;
      }
      if (action === 'voice') {
        ...
        if (operation === 'claim') {
          ...
          if (operation === 'heartbeat') ctx.state.leaseExpiresAt = new Date(Date.now() + LEASE_MS).toISOString();
            ...
            if (operation === 'speaking' && turn.phase === 'ready') {
              if ((ctx.interview.aiInterview.transcript?.length || 0) >= 2000) reject('CAPACITY', '本次面试已达到 2000 条记录上限，原记录保留', 413);
      ...
      if (action === 'speech') {
        fields(body, ['participantSessionId','requestId','text','questionId','kitId','preparedAudioKey','followup']);
        ...
        if (body.preparedAudioKey !== undefined && (typeof body.preparedAudioKey !== 'string' || !/^[a-f0-9]{64}:[a-f0-9]{64}$/.test(body.preparedAudioKey))) reject('VALIDATION', '已准备语音编号无效', 400);
        if (body.followup !== undefined && typeof body.followup !== 'boolean') reject('VALIDATION', '追问类型无效', 400);
        if (body.followup && body.preparedAudioKey !== undefined) reject('VALIDATION', '自定义追问不能复用题库语音', 400);
        const canonical = Boolean(!body.followup && questionId && kitId && bankQuestion(kitId, questionId) === text);
        if ((questionId || kitId) && !canonical && !body.followup) reject('VALIDATION', '题库提问必须使用该题的英文原文；自定义追问请单独发送', 400);
        const fingerprint = hash(JSON.stringify([text, questionId, kitId, ...(body.followup ? ['followup'] : [])])), previous = requestRecord(ctx.state, 'speech', id, fingerprint);
        ...
        let audio;
        try { audio = canonical ? await speechCache.prepare(text) : await voiceProvider.speech({ text }); decodeAudio(audio?.audioBase64); if (audio?.mimeType !== 'audio/mpeg') throw safeProviderError({ code:'INTERVIEW_VOICE_PROVIDER_INVALID' }); }
        catch (error) { throw saveFailure(guard, id, error); }
        ctx = after(guard, 'preparing');
        if (!configured()) throw saveFailure(guard, id, failure('NOT_CONFIGURED', 'AI 语音服务配置已改变', 503));
        ctx.state.currentTurn.phase = 'ready'; ctx.state.requests.find(row => row.id === id).status = 'complete';
        save(ctx.db, 'recruiting-voice-speech-ready', interviewId);
        const audioPrepared = Boolean(canonical && body.preparedAudioKey && body.preparedAudioKey === audio.audioKey);
        send(res, 200, { ...(audioPrepared ? { audioPrepared:true } : { audioBase64:audio.audioBase64 }), ...(audio.audioKey ? { audioKey:audio.audioKey } : {}),
          mimeType:'audio/mpeg', voiceName:SPEECH_PROFILE.voice, turnId:turn.id, requestStatus:'complete', ...response(ctx) }); return true;
      }
      ...
      let transcription;
      try { transcription = await voiceProvider.transcribe({ audio, mimeType:type }); if (typeof transcription?.text !== 'string' || !transcription.text.trim() || transcription.text.length > 12000) reject('VALIDATION', '回答转写为空或无效', 400); }
      catch (error) { throw saveFailure(guard, id, error); }
      ctx = after(guard, 'transcribing');
      if ((ctx.interview.aiInterview.transcript?.length || 0) >= 2000) throw saveFailure(guard, id, failure('CAPACITY', '本次面试已达到 2000 条记录上限，原记录保留', 413));
      const row = { id:`voice-answer-${uid()}`, speaker:'candidate', speakerName:ctx.candidate.name || 'Candidate', participantIdentity:`candidate-${ctx.invite.id}`,
        text:transcription.text.trim(), language:/\p{Script=Han}/u.test(transcription.text) ? 'zh' : 'en', translationZh:'', source:'openai_audio', turnId:turn.id, questionText:turn.questionText, questionId:turn.questionId, kitId:turn.kitId, createdAt:new Date().toISOString() };
      ctx.interview.aiInterview.transcript ||= []; ctx.interview.aiInterview.transcript.push(row);
