(() => {
  ...
  let analysisBusy = false, newEvidenceCount = 0, lastAutoAnalysisAt = 0;
  ...
  function renderReady() {
    ...
    renderTranscripts(info.aiState?.transcript || []); renderAnalysis(info.aiState?.analysis || null);
  }
  function ready(data) { info = data; $('aiPanel').hidden = !recruiter; renderReady(); }
  async function boot() {
    try {
  ...
  function renderList(title, values) {
    ...
    const list = document.createElement('ul'); values.forEach(value => { const item = document.createElement('li'); item.textContent = value; list.appendChild(item); });
