...
function salesOrderPageHtml(account,visit,head){const products=state.fieldSales?.products||[];const p1=products[0]||{sku:'QD15-BLK-15218',name:'QD15 陶瓷隔热膜'};return salesPage('现场订单',`${head}<section class="sales-order-data"><label>订单号<input readonly value="保存后自动生成"></label><label>业务员 · 可选可填<input value="${escapeHtml(user?.name||'')}"></label><label>订单类型 · 可选可填<input id="salesOrderType" value="批发订单"></label><label>出货仓库 · 可选可填<input id="salesOrderWarehouse" value="洛杉矶仓"></label></section><section class="sales-tool-card"><h3>订单产品 <button type="button">＋ 选择或输入产品</button></h3>${salesProductDatalist()}<article class="sales-order-product"><label>产品型号<input id="salesOrderSku" list="salesProductModels" value="${escapeHtml(p1.sku||'')}"></label><small>${escapeHtml(p1.name||p1.sku||'')} · 系统产品库</small><div class="sales-order-values"><label>数量<input id="salesOrderQty" type="number" value="6"></label><label>成交单价<input id="salesOrderPrice" type="number" value="340"></label><label>折扣<input id="salesOrderDiscount" type="number" value="60"></label></div></article></section><section class="sales-tool-card"><h3>交付与付款</h3><div class="sales-two"><label>交付方式 · 可选可填<input id="salesDeliveryMethod" value="现场交货"></label><label>交付日期<input type="date" value="${salesLocalDate()}"></label></div><div class="sales-two"><label>本次已收<input id="salesOrderPaid" type="number" value="1000"></label><label>付款方式 · 可选可填<input id="salesOrderPayment" value="信用卡"></label></div><div class="sales-two"><label>余款到期日<input id="salesOrderDue" type="date" value="${salesLocalDate(8)}"></label><label>税务状态<input id="salesOrderTax" value="批发免税"></label></div><label>订单备注<textarea id="salesOrderNote" placeholder="补充配送、价格或客户要求……"></textarea></label></section><section class="sales-tool-card"><h3>订单金额</h3><div class="sales-order-total"><p>产品小计 <b>$2,040.00</b></p><p>订单折扣 <b>− $60.00</b></p><p>销售税 <b>$0.00</b></p><p>订单总额 <b>$1,980.00</b></p></div><div class="sales-receipt-totals"><p>已收<b>$1,000</b></p><p>欠款<b>$980</b></p><p>产品<b>6 卷</b></p></div></section><button class="sales-page-primary" onclick="saveSalesFieldOrder(this,'${account.id}','${visit?.id||''}')">创建订单并进入客户签收</button><p class="sales-page-note">创建后自动生成库存出库、收款记录和客户应收账款。</p>`);}
...
function salesVisitToolHtml(screen){const {account,visit}=salesActiveContext();if(!account)return salesTodayHtml();const head=`<section class="sales-tool-customer"><h2>${escapeHtml(account.businessName)}</h2><span>本次拜访 · ${visit?.startedAt?new Date(visit.startedAt).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'已到店'}</span></section>`;
  ...
  if(screen==='followup')return salesPage('下次跟进',`${head}<section class="sales-form-card"><label>跟进方式<input value="到店拜访"></label><label>任务类型<input value="样品测试回访"></label><label>跟进时间<input type="datetime-local" value="${localDateTimeValue(new Date(Date.now()+7*86400000).toISOString())}"></label><label>跟进原因<textarea></textarea></label></section><button class="sales-page-primary" onclick="openSalesFollowUp('${account.id}')">保存跟进任务</button>`);
