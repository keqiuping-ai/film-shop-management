...
function prospectTable() {
  ...
  return `<div class="table-wrap"><table><thead><tr><th>${t('date')}</th><th>${addedLabel}</th><th>${t('source')}</th><th>${t('customer')}</th><th>${t('vehicleNeed')}</th><th>${t('appointmentAt')}</th><th>${t('contactOwner')}</th><th>${t('intentLevel')}</th><th>${t('prospectStatus')}</th><th>${t('chatContext')}</th><th>${t('note')}</th><th></th></tr></thead><tbody>
  ${rows.map(item => {
    ...
    const importLine = [item.sourceDevice, item.importSource].filter(Boolean).join(' / ');
    return `<tr><td>${escapeHtml(item.date || '')}</td><td>${prospectTimeCell(item)}</td><td>${escapeHtml(item.source || '')}${importLine ? `<br><span class="note">${escapeHtml(importLine)}</span>` : ''}</td><td>${escapeHtml(item.customer || '')}<br><span class="note">${escapeHtml(item.phone || '')}</span></td><td>${escapeHtml(item.vehicle || '')}<br><span class="note">${escapeHtml(item.need || '')}</span></td><td>${appointment}</td><td>${rep ? escapeHtml(rep.name) : escapeHtml(item.ownerName || '') || t('unassigned')}</td><td>${prospectIntentPill(item.intentLevel)}</td><td>${prospectStatusPill(item.status)}</td><td>${escapeHtml(prospectConversationSummary(item))}</td><td>${escapeHtml(shortText(item.note || '', 70))}</td>${actionCell('Prospect','prospects',item.id)}</tr>`;
  }).join('')}
