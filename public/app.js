...
const permissionLabels = [
  ...
  ['fieldSalesPriceBronze', '业务员手机端查看铜牌价', 'View bronze pricing in field sales mobile'],
  ['fieldSalesPriceSilver', '业务员手机端查看银牌价', 'View silver pricing in field sales mobile'],
  ['fieldSalesPriceGold', '业务员手机端查看金牌价', 'View gold pricing in field sales mobile'],
  ['fieldSalesPriceStrategic', '业务员手机端查看超级战略合作伙伴价', 'View strategic partner pricing in field sales mobile'],
...
function messageUserDisplayName(item) {
    ...
    <div class="message-people">
      ...
      ${users.map(item => {
        ...
        return `<button class="message-person ${item.id === activeMessageUserId ? 'active' : ''}" onclick="selectMessageUser('${item.id}')">
          <span class="message-person-identity"><span class="message-avatar-with-badge">${userAvatarHtml(item, 'small')}${mentioned ? '<i class="message-at-badge">@</i>' : ''}</span><span>${escapeHtml(messageUserDisplayName(item))}</span></span>
...
function fieldSalesLocalizedStatus(label) {
  ...
  const translations = {
    ...
    '待仓库确认':'Pending warehouse confirmation', '已到达':'Arrived', '待完成':'Pending',
    '已付款':'Paid', '部分付款':'Partially paid', '待付款':'Payment due'
  };
...
function fieldSalesAttachmentAccountId(item, data) {
  if (item.accountId) return item.accountId;
  const visit = (data.visits || []).find(row => row.id === item.objectId);
  if (visit?.accountId) return visit.accountId;
  const plan = (data.visitPlans || []).find(row => row.id === item.objectId);
  if (plan?.accountId) return plan.accountId;
  return (data.accounts || []).some(row => row.id === item.objectId) ? item.objectId : '';
}

function fieldSalesDayCustomers(data, dayPlans, dayVisits) {
  const accountById = new Map((data.accounts || []).map(item => [item.id, item]));
  const result = [];
  const seen = new Set();
  [...dayPlans, ...dayVisits].forEach(item => {
    const accountId = item.accountId || '';
    if (!accountId || seen.has(accountId)) return;
    seen.add(accountId);
    result.push(accountById.get(accountId) || {
      id:accountId,
      businessName:item.businessName || (lang === 'zh' ? '历史客户' : 'Historical customer'),
      address:item.address || item.destination?.address || '',
      stage:item.status || ''
    });
  });
  return result;
...
function fieldSalesMoney(value) {
  return new Intl.NumberFormat(lang === 'zh' ? 'zh-CN' : 'en-US', { style:'currency', currency:'USD' }).format(Number(value || 0));
}

function fieldSalesCustomerPhotos(visits, attachments) {
  const photos = [];
  const seen = new Set();
  const add = (url, label, at) => {
    if (!url || seen.has(url)) return;
    seen.add(url);
    photos.push({ url, label, at });
  };
  visits.forEach(visit => {
    add(visit.checkIn?.photoUrl, lang === 'zh' ? '到店门头照片' : 'Storefront at check-in', visit.startedAt || visit.createdAt);
    (visit.evidencePhotoUrls || []).forEach(url => add(url, lang === 'zh' ? '现场照片' : 'Visit photo', visit.completedAt || visit.updatedAt));
    add(visit.ownerPhotoUrl, lang === 'zh' ? '客户现场合影' : 'Customer visit photo', visit.completedAt || visit.updatedAt);
  });
  attachments.filter(item => String(item.contentType || '').startsWith('image/')).forEach(item => add(item.url, item.fileName || (lang === 'zh' ? '现场照片' : 'Visit photo'), item.createdAt));
  return photos;
}

function fieldSalesCustomerProfileHtml(account, personId, selectedDate, data) {
  const accountId = account.id;
  const visits = (data.visits || []).filter(item => item.accountId === accountId).sort((a, b) => String(b.startedAt || b.createdAt || '').localeCompare(String(a.startedAt || a.createdAt || '')));
  ...
  const trials = (data.trialRolls || []).filter(item => item.accountId === accountId);
  ...
  return `<div class="field-sales-customer-profile">
    <section class="field-sales-customer-hero"><div><small>${lang === 'zh' ? '客户完整档案' : 'Complete customer profile'}</small><h2>${escapeHtml(account.businessName || '')}</h2><p>${escapeHtml(account.address || '—')}</p></div><div>${fieldSalesStatusPill(account.stage || '待拜访', 'info')}<button class="btn" onclick="openFieldSalesAccount('${accountId}')">${lang === 'zh' ? '编辑客户资料' : 'Edit customer'}</button></div></section>
    ...
    ${followUps.length ? `<section><h3>${lang === 'zh' ? `后续跟进（${followUps.length}）` : `Follow-ups (${followUps.length})`}</h3>${fieldSalesEmployeeMiniRows(followUps, '', item => `<div class="field-sales-lane-row"><span><b>${escapeHtml(item.reason || item.type || '')}</b><small>${fieldSalesDateTime(item.dueAt)}</small></span>${fieldSalesStatusPill(item.status || '—', 'info')}</div>`)}</section>` : ''}
    <p class="field-sales-customer-sync-note">${lang === 'zh' ? `当天关联：${dayAttachments.length} 个附件、${dayOrders.length} 张订单。历史资料均保留在本客户档案中。` : `Linked that day: ${dayAttachments.length} ${dayAttachments.length === 1 ? 'attachment' : 'attachments'} and ${dayOrders.length} ${dayOrders.length === 1 ? 'order' : 'orders'}. Historical records remain in this customer profile.`}</p>
  </div>`;
}

function openFieldSalesCustomerDetail(accountId, personId = '', selectedDate = '') {
  const data = state.fieldSales || {};
  const account = (data.accounts || []).find(item => item.id === accountId);
  if (!account) return alert(lang === 'zh' ? '找不到这个客户档案。' : 'Customer profile not found.');
  const date = /^\d{4}-\d{2}-\d{2}$/.test(selectedDate) ? selectedDate : fieldSalesEmployeeDate(personId || account.assignedUserId || '');
  openModal(lang === 'zh' ? '客户拜访完整档案' : 'Complete customer visit profile', fieldSalesCustomerProfileHtml(account, personId, date, data), closeModal);
  const save = document.getElementById('modalSave');
  if (save) { save.textContent = lang === 'zh' ? '关闭' : 'Close'; save.onclick = closeModal; }
...
function fieldSalesEmployeeLane(person, data) {
  ...
  const dayAccounts = fieldSalesDayCustomers(data, dayPlans, visits);
  ...
  return `<article class="field-sales-employee-lane ${selected ? 'selected' : ''} ${expanded ? 'expanded' : ''}">
    ...
    ${expanded ? `<div class="field-sales-employee-detail">
      ...
      <section><h4>${lang === 'zh' ? '当天客户（点击查看全部细节）' : 'Customers for the day (open full details)'}</h4>${fieldSalesEmployeeMiniRows(dayAccounts, lang === 'zh' ? '当天没有安排或拜访客户' : 'No customers scheduled or visited that day', account => `<button class="field-sales-lane-row action customer-entry" onclick="openFieldSalesCustomerDetail('${account.id}', '${personId}', '${selectedDate}')"><span><b>${escapeHtml(account.businessName || '')}</b><small>${escapeHtml(account.address || '')}</small></span><span class="field-sales-customer-open">${lang === 'zh' ? '照片 · 订货 · 沟通 · 历史 ›' : 'Photos · Orders · Conversations · History ›'}</span></button>`)}</section>
      ...
      <details><summary>${lang === 'zh' ? `当天工作日报（${reports.length}）` : `Daily summaries for the day (${reports.length})`}</summary>${fieldSalesEmployeeMiniRows(reports, lang === 'zh' ? '当天没有工作日报' : 'No daily summaries that day', report => `<div class="field-sales-lane-row stacked"><span><b>${escapeHtml(report.date || '')}</b><small>${fieldSalesDateTime(report.createdAt)}</small></span><p>${escapeHtml(report.summary || '—')}</p></div>`)}</details>
...
function salesOrderIsFullyPaidForStockOut(order) {
  const calc = orderCalc(order);
  const paymentStatus = String(order.paymentStatus || '').trim().toLowerCase();
  const status = String(order.status || '').trim();
  if (calc.total <= 0) return ['已付清', '已付款', '待出库', '已付款待出库'].includes(status);
  return Number(order.paid || 0) + 0.001 >= calc.total || ['paid', '已收清', '已付清', '已付款'].includes(paymentStatus);
}

function salesOrderHasPhysicalStock(order) {
  return salesOrderLineItems(order).some(line => !isCustomPrintedFilmSku(line.item));
}

function salesOrderHasCompleteStockOutForUi(order) {
  const lines = salesOrderLineItems(order).filter(line => !isCustomPrintedFilmSku(line.item));
  return Boolean(lines.length) && lines.every(line => {
    ...
    return shipped + 0.001 >= Number(line.qty || 0);
  });
}

function pendingStockOutOrders() {
  const finished = new Set(['已出库', 'shipped', 'delivered', 'completed', '已完成', '已取消', '取消', 'canceled', 'cancelled', '已退款', 'refunded']);
  return sortByDateDesc((state.salesOrders || []).filter(order => {
    if (finished.has(String(order.status || '').trim().toLowerCase())) return false;
    if (!salesOrderHasPhysicalStock(order) || salesOrderHasCompleteStockOutForUi(order) || !salesOrderIsFullyPaidForStockOut(order)) return false;
    return true;
  }));
}
...
async function confirmSalesOrderStockOut(orderId, branchId, orderNo, button, closeAfter = false) {
  ...
  if (button) { button.disabled = true; button.textContent = lang === 'zh' ? '正在出库…' : 'Processing…'; }
  try {
    ...
    showActionFeedback(`${lang === 'zh' ? '出库失败：' : 'Stock-out failed: '}${error.message}`, 'error');
    alert(error.message);
  }
}
...
function retailWholesaleSalesTable(orders = []) {
  const retailWholesaleOrders = (orders || []).filter(order => {
    const status = String(order.status || '').trim().toLowerCase();
    return !['已取消', '取消', 'canceled', 'cancelled'].includes(status);
  });
  const groups = new Map();
  retailWholesaleOrders.forEach(order => {
  ...
  return `<div class="table-wrap wholesale-customer-sales"><table><thead><tr><th>${lang === 'zh' ? '零售/批发客户' : 'Retail / wholesale customer'}</th><th>${lang === 'zh' ? '订单数' : 'Orders'}</th><th>${t('qty')}</th><th>${lang === 'zh' ? '销售额' : 'Sales'}</th><th>${t('paid')}</th><th>${lang === 'zh' ? '未收款' : 'Outstanding'}</th><th>${lang === 'zh' ? '最近订单' : 'Latest order'}</th><th>${t('status')}</th></tr></thead><tbody>
    ...
    ${rows.length ? `<tr class="total-row"><td>${lang === 'zh' ? '合计' : 'Total'}</td><td>${totals.orders}</td><td>${Number(totals.qty || 0).toLocaleString()}</td><td>${currency.format(totals.total)}</td><td>${currency.format(totals.paid)}</td><td>${currency.format(totals.balance)}</td><td colspan="2"></td></tr>` : `<tr><td colspan="8" class="note">${lang === 'zh' ? '当前筛选时间内还没有零售批发订单。' : 'No retail / wholesale sales in the selected period.'}</td></tr>`}
  </tbody></table></div><p class="note">${lang === 'zh' ? '按客户汇总“零售批发”模块中当前筛选时间内的全部订单；取消订单不计入销售额。' : 'All orders from Retail / Wholesale are grouped by customer for the selected period; canceled orders are excluded.'}</p>`;
}
...
function portalPriceTierView() {
  ...
  const columns = [
    ['standard', lang==='zh'?'批发价':'Wholesale'],
    ['first-order', lang==='zh'?'首次进货价':'First-order'],
    ['bronze', lang==='zh'?'铜牌价':'Bronze'],
    ['silver', lang==='zh'?'银牌价':'Silver'],
    ['gold', lang==='zh'?'金牌价':'Gold'],
    ['strategic', lang==='zh'?'超级战略合作伙伴价':'Strategic partner']
  ];
  const tierById = id => tiers.find(item => item.id === id) || { prices: {} };
  const rows=(state.products||[]).map(product=>`<tr><td><strong>${escapeHtml(product.sku)}</strong><br><span class="note">${escapeHtml(product.name||'')}</span></td>${columns.map(([id])=>`<td><input class="portal-table-price" data-tier="${id}" data-sku="${escapeHtml(product.sku)}" type="number" min="0" step="0.01" value="${tierById(id).prices?.[product.sku]??''}" placeholder="${id==='standard'?(lang==='zh'?'请输入批发价':'Enter wholesale'):(lang==='zh'?'继承批发价':'Use wholesale')}"></td>`).join('')}</tr>`).join('');
  return `<div class="portal-tier-rules"><strong>${lang==='zh'?'客户等级规则':'Customer tier rules'}</strong><span>${lang==='zh'?'本月实付满 $10,000 升银牌；满 $20,000 升金牌。连续两个完整月份未达到当前等级门槛时仅降一级，金牌不会直接降为普通客户。':'Silver at $10,000 paid monthly sales; Gold at $20,000. Two completed months below the current threshold cause only a one-level downgrade.'}</span>${hasPerm('portalPricingEdit')?`<button class="btn" onclick="recalculatePortalTiers()">${lang==='zh'?'重新计算客户等级':'Recalculate tiers'}</button>`:''}</div><div class="table-wrap portal-price-matrix"><table><thead><tr><th>SKU</th>${columns.map(([,label])=>`<th>${label}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div>${hasPerm('portalPricingEdit')?`<button class="btn primary portal-price-save" onclick="savePortalPriceMatrix()">${lang==='zh'?'保存全部价格':'Save all prices'}</button>`:''}<p class="note">${lang==='zh'?'批发价是默认售价；其他等级留空时自动继承批发价。客户特殊协议价优先，订单生成时锁定成交价。':'Wholesale is the default; blank tier prices inherit it. Customer overrides take priority, and order prices are locked when created.'}</p>`;
}

async function savePortalPriceMatrix(){const payload={'standard':{},'first-order':{},bronze:{},silver:{},gold:{},strategic:{}};document.querySelectorAll('.portal-table-price').forEach(input=>{if(input.value!=='')payload[input.dataset.tier][input.dataset.sku]=Number(input.value)});try{const result=await api('/api/portal-price-tiers',{method:'PUT',body:JSON.stringify(payload)});state=result.data;render();broadcastDataChange();alert(lang==='zh'?'全部价格已保存。':'All prices saved.')}catch(err){alert(err.message)}}
async function recalculatePortalTiers(){if(!confirm(lang==='zh'?'系统将按已入账销售额重新计算所有客户等级；降级只会降一级。是否继续？':'Recalculate all customer tiers from paid sales?'))return;try{const result=await api('/api/portal-customers/recalculate-tiers',{method:'POST',body:'{}'});state=result.data;render();broadcastDataChange();alert(lang==='zh'?`计算完成，${result.changes.length} 个客户等级发生变化。`:`Done. ${result.changes.length} tiers changed.`)}catch(err){alert(err.message)}}
...
function portalTierProductPrice(tierId, product) {
  const tiers = state.portalPriceTiers || [];
  const selectedTier = tiers.find(tier => tier.id === (tierId || 'standard'));
  const standardTier = tiers.find(tier => tier.id === 'standard');
  const selectedPrice = Number(selectedTier?.prices?.[product.sku]);
  if (Number.isFinite(selectedPrice) && selectedPrice > 0) return selectedPrice;
  const wholesalePrice = Number(standardTier?.prices?.[product.sku]);
  if (Number.isFinite(wholesalePrice) && wholesalePrice > 0) return wholesalePrice;
  const productWholesale = Number(product.wholesale);
  return Number.isFinite(productWholesale) && productWholesale > 0 ? productWholesale : null;
}

function portalTierPriceLabel(tierId, product) {
  const price = portalTierProductPrice(tierId, product);
...
function updatePortalCustomerTierPreview(tierId) {
  const productsBySku = new Map((state.products || []).map(product => [String(product.sku), product]));
  document.querySelectorAll('.portal-inherited-price').forEach(cell => {
    const product = productsBySku.get(String(cell.dataset.sku || ''));
    if (product) cell.textContent = portalTierPriceLabel(tierId, product);
  });
}
    ...
    .map((message, index) => ({
      ...
      role: prospectIsYelpLeadFormMessage(message, item?.source) || prospectIsYelpSystemNotificationMessage(message, item?.source)
        ...
        : String(message.direction || '').toLowerCase() === 'outbound'
          ? 'shop'
          : prospectSpeakerRole(message.speaker || message.role || message.senderType, message.speakerName || message.name || ''),
      at: new Date(message.timestamp || message.time || message.createdAt || 0).getTime()
    }))
      ...
      <section class="prospect-workspace-conversation">
        ...
        <footer class="prospect-workspace-composer">
          ...
          <div class="prospect-attachment-tools">
            ...
            <select class="prospect-address-picker" aria-label="${lang === 'zh' ? '选择分店地址' : 'Choose branch address'}" title="${lang === 'zh' ? '选择一个分店地址并加入回复' : 'Choose one branch address and add it to the reply'}" onchange="insertProspectAddress(this.value);this.value=''">
...
function collectionName(collection) {
  const names = {
    ...
    movements: lang === 'zh' ? '库存单据' : 'Inventory Documents',
...
let jobConfirmationPhotoDraft = [];
let jobConfirmationPhotoUploading = false;

const tintConfirmationParts = [
  ['windshield','Windshield','前挡风玻璃'], ['frontLeft','Left front window','左前门玻璃'], ['frontRight','Right front window','右前门玻璃'],
  ['rearLeft','Left rear window','左后门玻璃'], ['rearRight','Right rear window','右后门玻璃'], ['rearWindow','Rear window','后挡风玻璃'],
  ['sunroof','Sunroof','天窗'], ['quarterWindows','Quarter windows','三角窗'], ['removeOldTint','Remove old tint','拆除旧膜']
];
const ppfConfirmationParts = [
  ['fullFront','Full front (bumper, hood and both fenders)','前脸套装（前杠＋引擎盖＋左右叶子板）'], ['fullVehicle','Full vehicle','全车'],
  ['frontBumper','Front bumper','前保险杠'], ['hood','Hood','引擎盖'], ['leftFender','Left fender','左叶子板'], ['rightFender','Right fender','右叶子板'],
  ['mirrors','Mirrors','后视镜'], ['doors','Doors','车门'], ['rockers','Rocker panels','侧裙'], ['roof','Roof','车顶'],
  ['rearQuarters','Rear quarter panels','后叶子板'], ['trunk','Trunk lid','后备箱盖'], ['rearBumper','Rear bumper','后保险杠']
];

function confirmationOptionLabel(option, printLanguage = 'bilingual') {
  const [, en, zh] = option;
  return printLanguage === 'en' ? en : printLanguage === 'zh' ? zh : `${zh} / ${en}`;
}

function confirmationCheckList(name, options, selected = []) {
  const values = new Set(Array.isArray(selected) ? selected : []);
  return `<div class="confirmation-check-grid">${options.map(option => `<label><input type="checkbox" name="${name}" value="${option[0]}" ${values.has(option[0]) ? 'checked' : ''} onchange="refreshConfirmationDiagrams()"><span>${escapeHtml(confirmationOptionLabel(option))}</span></label>`).join('')}</div>`;
}
...
function openJobConfirmation(jobId) {
  ...
  jobConfirmationPhotoDraft = Array.isArray(confirmation.damagePhotos) ? confirmation.damagePhotos.map(photo => ({ ...photo })) : [];
  openModal(lang === 'zh' ? '客户施工确认单' : 'Customer Installation Confirmation', `
    <div class="confirmation-editor-intro">
      ...
      <span>${lang === 'zh' ? '勾选施工位置并填写必要说明；保存后先查看 A4 预览，确认无误后再选择 Brother HL-L2465DW、A4、纵向、100%比例。' : 'Select installation areas, review the A4 preview, then print to Brother HL-L2465DW on A4 portrait at 100% scale.'}</span>
    </div>
    <div class="confirmation-editor-grid confirmation-top-grid">
      ...
      <label>打印语言 / Print language<select id="confirmPrintLanguage" onchange="refreshConfirmationDiagrams()"><option value="en" ${(confirmation.printLanguage || 'en') !== 'zh' ? 'selected' : ''}>English</option><option value="zh" ${confirmation.printLanguage === 'zh' ? 'selected' : ''}>中文</option></select></label>
      <label class="span-3">${lang === 'zh' ? '特殊配置（雷达、摄像头、电子锁等）' : 'Special equipment'}<input id="confirmSpecialEquipment" value="${escapeHtml(confirmation.specialEquipment || '')}"></label>
    </div>
    ${showTint ? `<section class="confirmation-editor-section"><h4>窗膜施工位置 / WINDOW TINT</h4><div class="confirmation-selection-layout"><div id="confirmTintDiagram">${tintVehicleDiagram(confirmation.tintParts, true, confirmation.printLanguage === 'zh' ? 'zh' : 'en')}</div>${confirmationCheckList('confirmTintPart', tintConfirmationParts, confirmation.tintParts)}</div>
    ...
    ${showPpf ? `<section class="confirmation-editor-section"><h4>PPF施工位置 / PAINT PROTECTION FILM</h4><div class="confirmation-selection-layout"><div id="confirmPpfDiagram">${ppfVehicleDiagram(confirmation.ppfParts, true, confirmation.printLanguage === 'zh' ? 'zh' : 'en')}</div>${confirmationCheckList('confirmPpfPart', ppfConfirmationParts, confirmation.ppfParts)}</div>
    ...
    <section class="confirmation-editor-section"><h4>施工要求与风险 / REQUIREMENTS & RISKS</h4>
      <div class="confirmation-editor-grid compact">
        ...
        <label class="span-2">原车状况及风险 / Existing damage & risks<textarea id="confirmRisks" rows="2">${escapeHtml(confirmation.risks || '')}</textarea></label>
        <div class="span-2 confirmation-photo-block"><div class="confirmation-photo-head"><b>原车刮蹭照片 / Damage photos</b><button type="button" class="btn" onclick="document.getElementById('confirmDamagePhotoInput').click()">📷 拍照 / 上传</button><input id="confirmDamagePhotoInput" type="file" accept="image/*" capture="environment" multiple hidden onchange="uploadJobDamagePhotos(event)"></div><div id="confirmDamagePhotoList"></div></div>
        <label class="span-2">其他说明 / Other notes<textarea id="confirmNotes" rows="2">${escapeHtml(confirmation.notes || job.notes || '')}</textarea></label>
      </div>
    </section>
    ${showTint ? `<aside class="confirmation-legal-notice"><strong>⚠️ 窗膜透光率及法规责任确认 / WINDOW TINT LEGALITY & VLT ACKNOWLEDGMENT</strong><p>客户理解车辆玻璃贴膜的透光率（VLT）、反光度、施工部位及其他要求会因车辆登记地、行驶地以及州或地方规定而不同。客户应自行确认所选择的膜系列、透光率和施工部位符合适用法律，并在知情情况下要求门店按本确认单施工。因客户选择或车辆在其他司法辖区使用而产生的检验不合格、罚款、处罚、整改、拆除或重新施工等责任和费用，由客户在法律允许范围内承担。门店提供的信息仅供一般参考，不构成法律合规保证；本条款不排除法律规定不得免除的门店责任。</p><p>The customer understands that requirements for visible light transmission (VLT), reflectivity, covered glass areas, and other tint specifications vary by vehicle registration, place of operation, and state or local law. The customer must independently confirm that the selected film, VLT, and installation areas comply with applicable law and knowingly requests installation as shown on this confirmation. To the extent permitted by law, the customer is responsible for inspection failures, citations, penalties, corrective work, removal, or reinstallation arising from the customer’s selection or use in another jurisdiction. Information provided by the shop is general information only and is not a guarantee of legal compliance. Nothing in this acknowledgment waives responsibilities that cannot lawfully be waived.</p></aside>` : ''}`, () => saveAndPrintJobConfirmation(jobId));
  document.getElementById('modal')?.classList.add('confirmation-modal-open');
  renderJobDamagePhotos();
  const save = document.getElementById('modalSave');
  if (save) save.textContent = lang === 'zh' ? '保存并预览 A4' : 'Save & Preview A4';
}

function refreshConfirmationDiagrams() {
  const tint = document.getElementById('confirmTintDiagram');
  const ppf = document.getElementById('confirmPpfDiagram');
  const printLanguage = document.getElementById('confirmPrintLanguage')?.value || 'en';
  if (tint) tint.innerHTML = tintVehicleDiagram(checkedConfirmationValues('confirmTintPart'), true, printLanguage);
  if (ppf) ppf.innerHTML = ppfVehicleDiagram(checkedConfirmationValues('confirmPpfPart'), true, printLanguage);
}

function toggleConfirmationPart(name, value) {
  const box = document.querySelector(`input[name="${name}"][value="${value}"]`);
  if (!box) return;
  box.checked = !box.checked;
  refreshConfirmationDiagrams();
}

function renderJobDamagePhotos() {
  const root = document.getElementById('confirmDamagePhotoList');
  if (!root) return;
  root.innerHTML = jobConfirmationPhotoDraft.length ? `<div class="confirmation-photo-grid">${jobConfirmationPhotoDraft.map((photo, index) => `<figure><img src="${escapeHtml(photo.url || '')}" alt="${escapeHtml(photo.name || 'Damage photo')}"><button type="button" aria-label="删除" onclick="removeJobDamagePhoto(${index})">×</button><figcaption>${escapeHtml(photo.name || `照片 ${index + 1}`)}</figcaption></figure>`).join('')}</div>` : '<span class="confirmation-photo-empty">尚未上传照片 / No photos uploaded</span>';
}

function removeJobDamagePhoto(index) {
  jobConfirmationPhotoDraft.splice(index, 1);
  renderJobDamagePhotos();
}

async function uploadJobDamagePhotos(event) {
  const input = event?.target;
  const files = [...(input?.files || [])];
  if (!files.length || jobConfirmationPhotoUploading) return;
  if (jobConfirmationPhotoDraft.length + files.length > 8) return alert('最多保存 8 张原车照片 / Maximum 8 photos');
  jobConfirmationPhotoUploading = true;
  const save = document.getElementById('modalSave');
  ...
  try {
    ...
    jobConfirmationPhotoUploading = false;
...
async function saveAndPrintJobConfirmation(jobId) {
  ...
  confirmation.printLanguage = document.getElementById('confirmPrintLanguage')?.value || 'en';
  confirmation.damagePhotos = jobConfirmationPhotoDraft.map(photo => ({ name: photo.name, type: photo.type, size: photo.size, url: photo.url }));
  try {
    ...
    closeModal();
    render();
    showJobConfirmationPrintPreview(savedJob);
  } catch (err) {
...
function confirmationSelectedLabels(selected, options, printLanguage) {
  return options.filter(([value]) => selected?.includes(value)).map(option => confirmationOptionLabel(option, printLanguage)).join(printLanguage === 'zh' ? '；' : '; ') || '—';
}

function confirmationPrintText(value, printLanguage, fallback = '—') {
  const text = String(value ?? '').trim();
  if (!text) return fallback;
  if (printLanguage !== 'en' || !/[\u3400-\u9fff]/u.test(text)) return text;
  const englishOnly = text
    .replace(/[\u3400-\u9fff]+/gu, ' ')
    .replace(/[\uff0c\u3002\uff1b\uff1a\u3001\uff01\uff1f\uff08\uff09]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  return /[A-Za-z0-9]/.test(englishOnly) ? englishOnly : fallback;
}

function confirmationFilmName(value, printLanguage) {
  const names = {
    '基本款': ['Standard', '基本款'],
    '标准款': ['Standard', '标准款'],
    '高级款': ['Premium', '高级款'],
  ...
  const pair = names[String(value || '').trim()];
  return pair ? pair[printLanguage === 'zh' ? 1 : 0] : confirmationPrintText(value, printLanguage);
}
...
function renderJobConfirmationPrint(job) {
  ...
  const printLanguage = c.printLanguage === 'zh' ? 'zh' : 'en';
  ...
  root.innerHTML = `<article class="confirmation-print-page">
    <header class="confirmation-print-head"><img src="/quad-film-icon.png" alt="QUAD FILM"><div><h1>${L('INSTALLATION AGREEMENT & SERVICE ACCEPTANCE','客户施工确认单')}</h1><p>QUAD FILM</p></div><div class="confirmation-order-no">${L('JOB #','施工单 #')} ${escapeHtml(job.id?.slice(-8).toUpperCase() || '')}</div></header>
    <section class="confirmation-info-grid">
      <div><b>${L('Customer','客户')}</b><span>${escapeHtml(P(job.customer, L('Customer','客户')))}</span></div><div><b>${L('Phone','电话')}</b><span>${escapeHtml(P(job.phone))}</span></div><div><b>${L('Date','日期')}</b><span>${escapeHtml(P(job.scheduleDate || job.date))}</span></div>
      <div><b>${L('Vehicle','车辆')}</b><span>${escapeHtml(P(job.vehicle, L('Vehicle not specified','未填写车型')))}</span></div><div><b>${L('Plate / VIN','车牌 / VIN')}</b><span>${escapeHtml(P(c.vin || job.vin))}</span></div><div><b>${L('Color','颜色')}</b><span>${escapeHtml(P(c.vehicleColor))}</span></div>
      <div class="span-3"><b>${L('Special equipment','特殊配置')}</b><span>${escapeHtml(P(c.specialEquipment, L('None','无')))}</span></div>
    </section>
    <div class="confirmation-service-layout ${tint && ppf ? 'two' : 'one'}">
      ${tint ? `<section class="confirmation-print-section"><h2>${L('WINDOW TINT AREAS','窗膜施工范围')}</h2>${tintVehicleDiagram(c.tintParts, false, printLanguage)}<p class="selected-parts"><b>${L('Selected','已选')}${sep}</b> ${escapeHtml(confirmationSelectedLabels(c.tintParts, tintConfirmationParts, printLanguage))}</p><div class="confirmation-inline"><span><b>${L('Film','膜')}${sep}</b> ${escapeHtml(confirmationFilmName(c.tintFilm || job.package, printLanguage))}</span><span><b>VLT${sep}</b> ${escapeHtml(P(c.tintVlt))}</span></div></section>` : ''}
      ${ppf ? `<section class="confirmation-print-section"><h2>${L('PAINT PROTECTION FILM AREAS','漆面保护膜施工范围')}</h2>${ppfVehicleDiagram(c.ppfParts, false, printLanguage)}<p class="selected-parts"><b>${L('Selected','已选')}${sep}</b> ${escapeHtml(confirmationSelectedLabels(c.ppfParts, ppfConfirmationParts, printLanguage))}</p><div class="confirmation-inline"><span><b>${L('Film','膜')}${sep}</b> ${escapeHtml(confirmationFilmName(c.ppfFilm || job.package, printLanguage))}</span><span><b>${L('Finish','表面')}${sep}</b> ${c.ppfFinish === 'matte' ? L('Matte','哑面') : L('Gloss','亮面')}</span></div></section>` : ''}
      ${!tint && !ppf ? `<section class="confirmation-print-section"><h2>${L('SERVICE','施工项目')}</h2><p>${escapeHtml(strictServiceLabel)} · ${escapeHtml(job.package || '')}</p></section>` : ''}
    </div>
    <section class="confirmation-print-section confirmation-requirements"><h2>${L('REQUIREMENTS & VEHICLE CONDITION','施工要求与原车状况')}</h2><div class="confirmation-four"><span>${c.disassembly === 'yes' ? '☑' : '☐'} ${L('Disassembly','拆件')}</span><span>${c.bladeFree !== 'no' ? '☑' : '☐'} ${L('Blade-free','无刀施工')}</span><span><b>${L('Edge wrap','包边')}${sep}</b>${escapeHtml(P(c.edgeWrap))}</span><span><b>${L('Delivery','交车')}${sep}</b>${escapeHtml(P(c.deliveryDate))}</span></div><p><b>${L('Existing damage / Risks','原车划痕及风险')}${sep}</b>${escapeHtml(P(c.risks, L('See internal job notes','未记录')))}</p><p><b>${L('Damage photos stored with job','原车照片已随施工单保存')}${sep}</b>${Number(c.damagePhotos?.length || 0)}</p><p><b>${L('Other notes','其他说明')}${sep}</b>${escapeHtml(P(c.notes))}</p></section>
    <section class="confirmation-payment"><span><b>${L('Quoted price','报价')}:</b> ${currency.format(Number(job.price || 0))}</span><span><b>${L('Deposit','订金')}:</b> ${currency.format(Number(job.deposit || 0))}</span><span><b>${L('Balance','余款')}:</b> ${currency.format(Math.max(0, Number(job.price || 0) - Number(job.paidAmount || job.deposit || 0)))}</span><span><b>${L('Payment','付款')}:</b> ${escapeHtml(strictPaymentName)}</span></section>
    <section class="confirmation-terms"><h2>${L('CUSTOMER ACKNOWLEDGEMENT','客户确认')}</h2><ol><li>${L('Vehicle condition, service areas, film type and shade shown above have been reviewed and approved.','客户已核对原车状况、施工部位、膜型及透光率。')}</li><li>${L('Film installation is a detailed handcraft service. Minor edge texture, small bubbles or silvering within industry tolerance may occur and are not defects.','贴膜属于精细手工施工，行业容许范围内的轻微边缘纹理、小气泡或银纹不属于质量问题。')}</li><li>${L('Customer authorizes the selected installation and any approved disassembly. Completed custom services and materials are non-refundable; all sales are final.','客户授权所选施工及已同意的拆装；定制施工完成后不退款。')}</li>${tint ? `<li class="confirmation-tint-liability"><b>${L('Window tint legality and VLT acknowledgment:','窗膜透光率及法规责任确认：')}</b> ${L('Tint requirements, including VLT, reflectivity and permitted glass areas, vary by state and locality. Customer has independently confirmed the selected film, VLT and installation areas comply with applicable law and knowingly requests this installation. To the extent permitted by law, customer is responsible for inspection failures, citations, penalties, corrective work, removal or reinstallation resulting from the selection or use in another jurisdiction. Shop information is general only and is not a guarantee of legal compliance. This does not waive responsibilities that cannot lawfully be waived.','窗膜透光率、反光度、允许施工部位等要求会因州和地方规定而不同。客户已自行确认所选膜系列、透光率和施工部位符合适用法律，并在知情情况下要求施工。因该选择或车辆在其他司法辖区使用而产生的检验不合格、罚款、处罚、整改、拆除或重新施工等责任和费用，由客户在法律允许范围内承担。门店信息仅供一般参考，不构成法律合规保证；本条款不排除法律规定不得免除的门店责任。')}</li>` : ''}<li>${L('This agreement is governed by Nevada law.','本确认单适用内华达州法律。')}</li></ol></section>
    <section class="confirmation-signatures"><div>${L('Customer Signature','客户签名')}${sep} <span></span></div><div>${L('Date','日期')}${sep} <span></span></div><div>${L('Advisor','接待')}${sep} <span>${escapeHtml(P(repName(job.receptionRepId) || job.preparedBy, L('QUAD Staff','店内员工')))}</span></div></section>
...
function showJobConfirmationPrintPreview(job) {
  renderJobConfirmationPrint(job);
  const printLanguage = job.confirmation?.printLanguage === 'zh' ? 'zh' : 'en';
  let overlay = document.getElementById('confirmationPrintPreview');
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = 'confirmationPrintPreview';
    overlay.className = 'confirmation-print-preview';
    document.body.appendChild(overlay);
  }
...
function closeJobConfirmationPrintPreview() {
  document.getElementById('confirmationPrintPreview')?.classList.remove('open');
...
function printJobConfirmationFromPreview() {
  ...
  try {
    ...
    return alert(lang === 'zh'
      ? '无法打开系统打印窗口，请按键盘 ⌘P 打开打印窗口。'
      : 'Unable to open the system print dialog. Press ⌘P to open it.');
  }
}
...
function openProduct(id) {
  ...
  const fields = [
    ...
    ['portalVisible',lang === 'zh' ? '客户客户端展示' : 'Show in customer portal','select',item.portalVisible === false ? 'false' : 'true',[['true',lang === 'zh' ? '展示' : 'Show'],['false',lang === 'zh' ? '隐藏' : 'Hide']]], ['portalPurchasable',lang === 'zh' ? '是否允许购买' : 'Purchasable','select',item.portalPurchasable === false ? 'false' : 'true',[['true',lang === 'zh' ? '允许购买' : 'Purchasable'],['false',lang === 'zh' ? '仅展示，禁止结账' : 'View only']]], ['portalNewProduct',lang === 'zh' ? '新品标签' : 'New product badge','select',item.portalNewProduct ? 'true' : 'false',[['false',lang === 'zh' ? '普通产品' : 'Regular'],['true',lang === 'zh' ? '新品' : 'New']]]
  ];
  const ids = ['sku','model','specification','name','category','unit', ...(canSeeFinance() ? ['cost'] : []), 'price','wholesale','minPrice','qty','reorder','location','portalDescription','portalImageUrl','portalVideoUrl','portalVisible','portalPurchasable','portalNewProduct'];
  openModal(id ? (lang === 'zh' ? '编辑库存商品' : 'Edit SKU') : (lang === 'zh' ? '新增库存商品' : 'New SKU'), formHtml(fields), () => { const data = numeric(readForm(ids), [...(canSeeFinance() ? ['cost'] : []), 'price','wholesale','minPrice','qty','reorder']); data.portalVisible = data.portalVisible === 'true'; data.portalPurchasable = data.portalPurchasable === 'true'; data.portalNewProduct = data.portalNewProduct === 'true'; return saveRecord('products', id, data); });
}

function openMovement(preset = {}) {
  openModal(preset.type === 'in' ? (lang === 'zh' ? '新增入库单' : 'New Stock-in') : (lang === 'zh' ? '新增出库单' : 'New Stock-out'), formHtml([
...
function openStockIn(preset = {}) {
  openModal(lang === 'zh' ? '新增入库单' : 'New Stock-in Record', formHtml([
    ['date',t('date'),'date',today()],
    ['branchId',lang === 'zh' ? '入库分店' : 'Receiving branch','select',preset.branchId || (inventoryBranchFilter !== 'all' ? inventoryBranchFilter : '') || defaultBranchId(),branchOptions(false)],
    ['sku','SKU','select',preset.sku || state.products[0]?.sku || '', state.products.map(product => [product.sku, `${product.sku} · ${product.name || ''}`])],
    ['qty',t('qty'),'number',preset.qty || 0],
    ['note',t('note'),'textarea',preset.note || '',null,'wide']
  ]), () => {
    const data = numeric(readForm(['date','branchId','sku','qty','note']), ['qty']);
    data.type = 'in';
    data.salesOrderId = '';
    const dateError = validateTodayEntryDate(data.date);
    if (dateError) return alert(dateError);
    if (data.qty <= 0) return alert(lang === 'zh' ? '入库数量必须大于 0。' : 'Stock-in quantity must be greater than 0.');
    return saveRecord('movements', null, data);
  });
}

function openMovementForSku(sku, type = 'in', qty = 1) {
  if (type === 'in') return openStockIn({
    sku,
    qty,
    note: lang === 'zh' ? '低库存补货入库' : 'Low-stock replenishment'
  });
  openMovement({
    sku,
    type,
