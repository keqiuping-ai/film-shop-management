...
function portalPriceTierView() {
  const tiers = state.portalPriceTiers || [], silver=tiers.find(x=>x.id==='silver')||{prices:{}}, gold=tiers.find(x=>x.id==='gold')||{prices:{}};
  const rows=(state.products||[]).map(product=>`<tr><td><strong>${escapeHtml(product.sku)}</strong><br><span class="note">${escapeHtml(product.name||'')}</span></td><td>${currency.format(Number(product.wholesale||0))}</td><td><input class="portal-table-price" data-tier="silver" data-sku="${escapeHtml(product.sku)}" type="number" min="0" step="0.01" value="${silver.prices?.[product.sku]??''}" placeholder="${lang==='zh'?'继承标准价':'Use standard'}"></td><td><input class="portal-table-price" data-tier="gold" data-sku="${escapeHtml(product.sku)}" type="number" min="0" step="0.01" value="${gold.prices?.[product.sku]??''}" placeholder="${lang==='zh'?'继承标准价':'Use standard'}"></td></tr>`).join('');
  return `<div class="portal-tier-rules"><strong>${lang==='zh'?'客户等级规则':'Customer tier rules'}</strong><span>${lang==='zh'?'本月实付满 $10,000 升银牌；满 $20,000 升金牌。连续两个完整月份未达到当前等级门槛时仅降一级，金牌不会直接降为普通客户。':'Silver at $10,000 paid monthly sales; Gold at $20,000. Two completed months below the current threshold cause only a one-level downgrade.'}</span>${hasPerm('portalPricingEdit')?`<button class="btn" onclick="recalculatePortalTiers()">${lang==='zh'?'重新计算客户等级':'Recalculate tiers'}</button>`:''}</div><div class="table-wrap portal-price-matrix"><table><thead><tr><th>SKU</th><th>${lang==='zh'?'标准价':'Standard price'}</th><th>${lang==='zh'?'银牌价':'Silver price'}</th><th>${lang==='zh'?'金牌价':'Gold price'}</th></tr></thead><tbody>${rows}</tbody></table></div>${hasPerm('portalPricingEdit')?`<button class="btn primary portal-price-save" onclick="savePortalPriceMatrix()">${lang==='zh'?'保存价格表':'Save price table'}</button>`:''}<p class="note">${lang==='zh'?'客户特殊协议价优先；等级价留空时自动采用标准价。订单生成时锁定成交价。':'Customer overrides take priority; blank tier prices inherit the standard price. Order prices are locked when created.'}</p>`;
}

async function savePortalPriceMatrix(){const payload={silver:{},gold:{}};document.querySelectorAll('.portal-table-price').forEach(input=>{if(input.value!=='')payload[input.dataset.tier][input.dataset.sku]=Number(input.value)});try{const result=await api('/api/portal-price-tiers',{method:'PUT',body:JSON.stringify(payload)});state=result.data;render();broadcastDataChange();alert(lang==='zh'?'价格表已保存。':'Price table saved.')}catch(err){alert(err.message)}}
